...
class AlexNet(nn.Module):
    def __init__(self, hash_bit, model_name='AlexNet', pretrained=True):
...
class ResNet(nn.Module):
    def __init__(self, hash_bit, model_name="ResNet101"):
        ...
        self.reduction_layer = nn.Linear(model_resnet.fc.in_features, 512)
        self.reduction_layer.weight.data.normal_(0, 0.01)
        self.reduction_layer.bias.data.fill_(0.0)
    ...
    def forward(self, x):
        ...
        y = self.hash_layer(x)
        return y
    def dist_forward(self, x):
        x = self.feature_layers(x)
        x = x.view(x.size(0), -1)
        return x
    def reduce_forward(self, x):
        x = self.feature_layers(x)
        x = x.view(x.size(0), -1)
        x = self.reduction_layer(x)
        return x
...
class VGG(nn.Module):
    def __init__(self, bit, model_name="VGG13"):
